<template>
  <div class="TransactionRow d-flex bg-light-subtle align-items-center border border-start-0 mx-2 mx-lg-0 bg-white pe-2">
    <span
      class="TransactionRow__indicator me-2"
      :style="{ 'background-color': transaction.category.color }"
    />
    <div class="TransactionRow__name flex-grow-1 py-2">
      {{ transaction.name }}
      <div class="fs-6 text-muted">
        {{ transaction.category.name }}
      </div>
    </div>
    <div class="TransactionRow__amount text-end">
...
.TransactionRow:not(:first-child) {
  margin-top: -1px;
}

.TransactionRow__indicator {
  width: 0.5rem;
  align-self: stretch;
  display: block;
}
